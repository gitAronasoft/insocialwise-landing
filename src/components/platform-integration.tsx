          ...
          <div className="relative h-[400px] w-full border-blue-600 mx-auto flex flex-col items-center justify-center border">
            ...
            {platforms.map((platform, index) => {
              ...
              return (
                ...
                >
                  <Card className={`group relative overflow-hidden bg-gradient-to-br ${platform.bgGradient} ${platform.hoverGradient} border border-white/50 shadow-lg ${platform.shadowColor} hover:shadow-xl transition-all duration-500 rounded-2xl backdrop-blur-sm`}>
                    ...
                    <CardContent className="p-6 flex flex-col items-center justify-center relative z-10">
                      <motion.div
                        ...
                        transition={{ duration: 0.3 }}
                      >
                        <platform.icon className={`w-8 h-8 ${platform.color} transition-all duration-300 group-hover:scale-110`} />
                      </motion.div>
                      <span className="text-xs font-semibold text-gray-700 mt-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                        {platform.name}
                      </span>
                    </CardContent>
                  </Card>
            ...
            {additionalPlatforms.map((platform, index) => {
              const totalIndex = platforms.length + index;
              const angle = (totalIndex * 360) / (platforms.length + additionalPlatforms.length);
              const radius = 180;
              const x = 50 + (Math.cos((angle * Math.PI) / 180) * radius) / 8;
              const y = 50 + (Math.sin((angle * Math.PI) / 180) * radius) / 4;
              ...
              return (
                <motion.div
                  ...
                  whileInView={{ opacity: 1, scale: 1 }}
                  viewport={{ once: true }}
                  transition={{ delay: 0.6 + totalIndex * 0.1 }}
                  whileHover={{ 
                    scale: 1.15,
                    y: -5,
                    transition: { duration: 0.3 }
                ...
                >
                  <Card className={`group relative overflow-hidden bg-gradient-to-br ${platform.bgGradient} ${platform.hoverGradient} border border-white/50 shadow-lg ${platform.shadowColor} hover:shadow-xl transition-all duration-500 rounded-2xl backdrop-blur-sm`}>
                    ...
                    <CardContent className="p-6 flex flex-col items-center justify-center relative z-10">
                      <span className={`text-xs font-bold ${platform.color} transition-all duration-300 group-hover:scale-110`}>
                        {platform.name}
                      </span>
                    </CardContent>
                  </Card>
